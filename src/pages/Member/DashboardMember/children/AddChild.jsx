...
Step2Page1.propTypes = {
  growthForm: PropTypes.shape({
    ...
    headCircumference: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.number,
    ]),
        ...
        <div className="two-column-row">
          ...
          <div>
            ...
            {errors.chestCircumference && (
              <p className="error-text">{errors.chestCircumference}</p>
            )}
          </div>
        </div>
        <div className="two-column-row">
...
Step2Page2.propTypes = {
  growthForm: PropTypes.shape({
    chestCircumference: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.number,
    ]),
...
Step2Page3.propTypes = {
  growthForm: PropTypes.shape({
    ...
    growthHormoneLevel: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.number,
    ]),
  ...
  useEffect(() => {
    setErrors((prevErrors) => {
      ...
      if (
        ...
        parseFloat(growthForm.headCircumference) < 0
      ) {
        newErrors.headCircumference =
          "Head circumference must not be less than 0";
      }
      if (
        growthForm.muscleMass !== "" &&
        parseFloat(growthForm.muscleMass) < 0
      ) {
        // Nếu cần validation, thêm tương tự
      }
      if (
        growthForm.chestCircumference !== "" &&
        parseFloat(growthForm.chestCircumference) < 0
      ) {
        newErrors.chestCircumference =
          "Chest circumference must not be less than 0";
      }
      if (
        growthForm.ferritinLevel !== "" &&
        parseFloat(growthForm.ferritinLevel) < 0
      ) {
        newErrors.ferritinLevel = "Ferritin level must not be less than 0";
      }
      if (
        growthForm.triglycerides !== "" &&
        parseFloat(growthForm.triglycerides) < 0
      ) {
        newErrors.triglycerides = "Triglycerides must not be less than 0";
      }
      if (
        growthForm.bloodSugarLevel !== "" &&
        parseFloat(growthForm.bloodSugarLevel) < 0
      ) {
      ...
      if (
        ...
        parseFloat(growthForm.growthHormoneLevel) < 0
      ) {
        newErrors.growthHormoneLevel =
          "Growth hormone level must not be less than 0";
      }
      ...
      if (growthForm.bodyTemperature !== "") {
        ...
        if (bt < 0) {
          newErrors.bodyTemperature =
            "Body temperature must not be less than 0";
        } else if (bt >= 45) {
      ...
      if (growthForm.oxygenSaturation !== "") {
        ...
        if (ox < 0) {
          newErrors.oxygenSaturation =
            "Oxygen saturation must not be less than 0";
        } else if (ox > 100) {
  ...
  const handleConfirmStep1 = useCallback(async () => {
    ...
    try {
      const childPayload = {
        ...
        dateOfBirth: new Date(childForm.dateOfBirth)
          .toISOString()
          .split("T")[0],
            ...
            {currentStep === 3 ? (
              ...
              <h2 className="congrats-text" style={{ color: "#000" }}>
                ...
                <span style={{ color: "#FF00CC" }}>
                  {childForm.name}
                </span> to <span style={{ color: "#00D0BC" }}>BabyHaven</span>
              </h2>
            ) : (
